"""
date: june 23, 2025
Copy of the dynamax ssm.py file that supports variable length sessions for linreg and logreg hmm classes.
"""
    ...
    * :meth:`fit_sgd` run SGD to minimize the *negative* marginal log probability.
    ...
    def fit_em_original(
        ...
        *Note:* ``emissions`` *and* ``inputs`` *can either be single sequences or batches of sequences.*
        ...
        print(emissions.shape, self.emission_shape, len(self.emission_shape))
        batch_emissions = ensure_array_has_batch_dim(emissions, self.emission_shape)
        batch_inputs = ensure_array_has_batch_dim(inputs, self.inputs_shape)

        @jit
        def em_step(params, m_step_state):
            # debug.print('e_step in-params {y}', y=params)
            batch_stats, lls = vmap(partial(self.e_step, params))(batch_emissions, batch_inputs)
            lp = self.log_prior(params) + lls.sum()
            # debug.print('e_step out-batch_stats: {x}', x=batch_stats)
            # debug.print('e_step out-lls: {x} {y}', x=lls, y=lp)
            # debug.print('e_step out-params/m_step in-params {y}', y=params)
            params, m_step_state = self.m_step(params, props, batch_stats, m_step_state)
            # debug.print('m_step out-params {y}', y=params)
        ...
        # print("before:", params)
        for _ in pbar:
            # debug.print("________________________________________________________ iter={x}", x=_)
            params, m_step_state, marginal_loglik = em_step(params, m_step_state)
            if jnp.isnan(marginal_loglik):
                break
            log_probs.append(marginal_loglik)
            # print(params)
        return params, jnp.array(log_probs)

    def fit_em_lrhmm_custom(
        self,
        params: ParameterSet,
        props: PropertySet,
        emissions: Union[Float[Array, "num_timesteps emission_dim"],
        ...
        """
        Custom version to support variable length sessions, for LinearRegressionHMM. the vmap stacking code is different for each class
        """
        ...
        def em_step(params, m_step_state):
            batch_stats_ = []
            lls_ = []
            for e, i in zip(batch_emissions, batch_inputs):
                bs, ll = self.e_step(params, e, i)
                batch_stats_.append(bs)
                lls_.append(ll)

            batch_stats = tree_map(lambda *xs: jnp.stack(xs), *batch_stats_)
            lls = jnp.stack(lls_)
    ...
    def fit_em_logrhmm_custom(
        ...
        """
        Custom version to support variable length sessions, for LogisticRegressionHMM. the vmap stacking code is different for each class
        """
        ...
        def em_step(params, m_step_state):
            ...
            lls_ = []
            for e, i in zip(batch_emissions, batch_inputs):
                if e.shape[-1] == 1:    # required for LogRegHMM since that only supports one emission, hence expects (T, ) and not (T, 1)
                    e = e.squeeze(-1)  # (7841, 1) → (7841,)
                bs, ll = self.e_step(params, e, i)
                batch_stats_.append(bs)
                lls_.append(ll)

            arr1s = jnp.stack([d[0] for d in batch_stats_])  # shape (N, 3)
            arr2s = jnp.stack([d[1] for d in batch_stats_])  # shape (N, 3, 3)
            arr3s = [(d[2][0], d[2][1], d[2][2]) for d in batch_stats_]  # shape (sum_T, 3)
            batch_stats = (arr1s, arr2s, arr3s)
            lls = jnp.stack(lls_)
